// render-quotes.js
// NOTE: debounce is now provided by render-list.js (_listDebounce) to avoid
// duplicate global declarations.  We keep a local alias here for clarity.
const _quotesDebounce = typeof _listDebounce === 'function'
    ? _listDebounce
    : (fn, d) => { let t; return (...a) => { clearTimeout(t); t = setTimeout(() => fn(...a), d); }; };
...
function assignInitialCustomOrders(allQuotes) {
    ...
    sortedByDate.forEach((q, index) => { q.originalQuote.customOrder = index; });
}
...
function getSortedQuotes(allQuotes) {
    ...
    } else if (mode === 'newest')   { sorted.sort((a, b) => (b.date || 0) - (a.date || 0)); }
    else if (mode === 'oldest')     { sorted.sort((a, b) => (a.date || 0) - (b.date || 0)); }
    else if (mode === 'title')      { sorted.sort((a, b) => a.book.title.localeCompare(b.book.title)); }
    else if (mode === 'author')     { sorted.sort((a, b) => (a.book.author || '').localeCompare(b.book.author || '')); }
    else if (mode === 'page')       { sorted.sort((a, b) => (a.page || 999999) - (b.page || 999999)); }
    else if (mode === 'favorite')   { sorted.sort((a, b) => (b.favorite ? 1 : 0) - (a.favorite ? 1 : 0)); }
...
function renderQuotes() {
        ...
        (book.quotes || []).forEach(quote => {
            if (quote.text?.trim()) {
                if (quote.id === undefined) {
                    quote.id = Date.now() + Math.random().toString(36).slice(2);
                }
                allQuotes.push({ ...quote, book, originalQuote: quote });
            }
        });
    });
    ...
    function renderFilteredQuotes() {
        ...
        if (selectedBookId !== "all")  filtered = filtered.filter(q => q.book.importOrder == selectedBookId);
        if (onlyFavorites)             filtered = filtered.filter(q => q.favorite);
        if (searchText) {
            filtered = filtered.filter(q =>
                q.text.toLowerCase().includes(searchText) ||
                q.book.title.toLowerCase().includes(searchText) ||
                (q.book.author || "").toLowerCase().includes(searchText)
            );
        }
        ...
        container.querySelectorAll(".cancel-quote-btn").forEach(btn => {
            btn.addEventListener("click", e => {
                e.target.closest(".edit-quote-form").style.display = "none";
            });
        });
        // Remove
        container.querySelectorAll(".remove-quote-btn").forEach(btn => {
            btn.addEventListener("click", e => {
                ...
                const id = e.target.closest(".quote-item").dataset.quoteId;
                books.forEach(book => { book.quotes = book.quotes?.filter(q => q.id !== id) || []; });
                saveBooksToLocal();
                renderQuotes();
            });
        });
        ...
        if (getQuotesSortMode() === 'custom') attachDragAndDrop(container);
    }
    ...
    document.getElementById("quoteSearchInput")?.addEventListener("input", _quotesDebounce(renderFilteredQuotes, 300));
...
function attachDragAndDrop(container) {
    ...
    const handleDragOver = _quotesDebounce(e => {
        e.preventDefault();
...
function getDragAfterElement(container, y) {
    const elements = [...container.querySelectorAll('.quote-item:not(.dragging)')];
...
function saveCustomOrder(container) {
    [...container.querySelectorAll('.quote-item')].forEach((item, newIndex) => {
        const id = item.dataset.quoteId;
        const originalQuote = books.flatMap(b => b.quotes || []).find(q => q.id === id);
        if (originalQuote) originalQuote.customOrder = newIndex;
    });
    saveBooksToLocal();
}
